...
function add_scroller() {
  ...
  if (document.querySelector('.navbar-content')) {
    const scroll = new SimpleBar(document.querySelector('.navbar-content'));
    scroll.getScrollElement().addEventListener('scroll', (evt) => {
      document.querySelector('.navbar-content').dispatchEvent(new CustomEvent('scroll', { detail: evt }))
    });
...
function layout_change(type, layout) {

  console.log('changing', type, layout)
  switch (type) {
    case 'theme':
      var control = document.querySelector('.pct-offcanvas');
      ...
      if (layout == 'dark') {
        document.querySelector('[data-tb-indicator="changeTheme"]').classList.remove('ti-sun')
        document.querySelector('[data-tb-indicator="changeTheme"]').classList.add('ti-moon')
        ...
        document.querySelector('[data-tb-indicator="changeTheme"]').classList.remove('ti-moon')
        document.querySelector('[data-tb-indicator="changeTheme"]').classList.add('ti-sun')
        dark_flag = false;
        if (document.querySelector('.pc-sidebar .m-header .logo-lg')) {
        ...
        layout_change('theme', dark_layout);
        var btn_control = document.querySelector('.theme-layout .btn[data-value="default"]');
        if (btn_control) {
          btn_control.classList.add('active');
        }
      ...
      setCookie('theme', layout, 36500000000)
...
function change_box_container(value) {
  ...
  setCookie('container', value, 36500000000)
}
...
function setCookie(name, value, options = {}) {
  ...
  AppConfig.settings.set(name, value)
  // Establecer una fecha de expiración en el futuro lejano (por ejemplo, 10 años)
  ...
  // Agregar otras opciones si están presentes
  if (options.domain) {
    cookieString += `; Domain=${options.domain || 'trelk.xyz'}`;
  }
  if (options.path) {
...
function getCookie(name) {
  ...
  for (let cookie of cookies) {
    // Eliminar espacios en blanco al inicio y al final de la cadena de cookie
    cookie = cookie.trim();

    // Separar el nombre y el valor de la cookie
    const [cookieName, cookieValue] = cookie.split('=');

    // Si el nombre de la cookie coincide con el nombre buscado, devuelve su valor
