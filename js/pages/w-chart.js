...
function floatchart() {
    (function () {
        ...
        // Primero, define los datos para el gráfico de barras
        var options12 = {
            chart: {
                height: 250,
                type: 'bar',
            ...
            xaxis: {
                categories: ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'HEAD', 'OPTIONS', 'TRACE']
            },
            tooltip: {
                y: {
        ...
        // Ahora, procesa los datos de "methods_stats" y actualiza el gráfico
        var methods_stats = document.querySelector('[data-tb-json="methods_stats"]');
        methods_stats = JSON.parse(decodeURIComponent(methods_stats.textContent.replace(/&#(\d+);/g, function (match, dec) {
            return String.fromCharCode(dec);
        })));
        console.log(methods_stats);
   
        // Actualiza los datos de la serie en el gráfico
        Object.keys(methods_stats).forEach(function (key) {
            options12.series[0].data.push(methods_stats[key].total_reqs);
            options12.series[1].data.push(methods_stats[key].success_reqs);
            options12.series[2].data.push(methods_stats[key].failed_reqs);
        });
        ...
        // Procesa los datos de "country_stats" y actualiza el gráfico
        var country_stats = {
            "CO": {
                "blocked_reqs": 55,
                "last_req": {
        ...
        // Actualiza los datos de la serie y las categorías del eje x
        Object.keys(country_stats).forEach(function (country) {
            options20.series[0].data.push(country_stats[country].total_reqs);
            options20.series[1].data.push(country_stats[country].success_reqs);
            options20.series[2].data.push(country_stats[country].failed_reqs);
            options20.xaxis.categories.push(country);
        });

        // Crea el gráfico con ApexCharts
        var chart = new ApexCharts(document.querySelector('#overview-chart-2'), options20);
        chart.render();
